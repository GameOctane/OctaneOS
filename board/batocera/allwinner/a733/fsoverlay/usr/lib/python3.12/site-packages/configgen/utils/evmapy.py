...
class evmapy(AbstractContextManager[None, None]):
    ...
    def __write_controller_config(self, controller: Controller, keys_actions: _KeysActions, keys_file: Path, /) -> None:
        device_path = controller.device_path
        if device_path is None:
            # device_path not set yet (controller monitor thread resolves async).
            # Parse VID/PID from the controller GUID and match to the right evdev
            # device — avoids picking the wrong controller in multi-player sessions.
            # Batocera GUID: 4-byte LE fields: bus | vendor | product | version
            # e.g. "030000005e0400008e02000014010000" → VID=0x045e PID=0x028e
            try:
                import evdev
                _VIRTUAL = {"evmapy", "batocera hotkeys"}
                guid = getattr(controller, 'guid', '') or ''
                target_vid: int | None = None
                target_pid: int | None = None
                if len(guid) >= 20:
                    try:
                        target_vid = int(guid[8:10], 16) | (int(guid[10:12], 16) << 8)
                        target_pid = int(guid[16:18], 16) | (int(guid[18:20], 16) << 8)
                    except ValueError:
                        pass
                for dev_path in evdev.list_devices():
                    try:
                        dev = evdev.InputDevice(dev_path)
                        if dev.name in _VIRTUAL or evdev.ecodes.EV_KEY not in dev.capabilities():
                            continue
                        if target_vid is not None and target_pid is not None:
                            if dev.info.vendor == target_vid and dev.info.product == target_pid:
                ...
                pass
        if device_path is None:
            _logger.warning("evmapy: no device_path for player %s, skipping hotkey config", controller.player_number)
            return
        config_file = _EVMAPY_RUN_DIR / f'{Path(device_path).name}.json'
            ...
            if input.type == 'button':
                ...
                if input.code not in known_axis_codes and input.code is not None:
                    ...
                    if (axis_id and axis_name) or axis_id == '_OTHERS_':
                        axis_min, axis_max = self.__get_pad_min_max_axis(device_path, int(input.code))
